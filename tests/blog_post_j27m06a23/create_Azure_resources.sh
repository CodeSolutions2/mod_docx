...
# Cognitive Services : FormRecognizer
...
# Cognitive Services : TextTranslation
...
then 
    ...
    export COGSERVname=$(echo "cog-servtexttranlation-$randomIdentifier")

    # sku FO = gratuit, S1=standard
    az cognitiveservices account create --name $COGSERVname \
                                        --resource-group $resourceGroupname \
    ...
    az cognitiveservices account keys list --name $COGSERVname \
                                       --resource-group $resourceGroupname

    # https://learn.microsoft.com/fr-fr/cli/azure/cognitiveservices/account/keys?view=azure-cli-latest#az-cognitiveservices-account-keys-list
    az cognitiveservices account keys list --name $COGSERVname -g $resourceGroupname --query key1
    export COGSERV_TextTranslation_KEY=$(az storage account keys list -g $resourceGroupname -n $STORAGE_NAME | jq '.[].value' |cut -d $'\n' -f 1)
    echo $COGSERV_TextTranslation_KEY
